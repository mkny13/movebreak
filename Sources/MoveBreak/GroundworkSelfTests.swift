...
enum GroundworkSelfTests {
    ...
    private static func runCompletionCases() -> Int {
        ...
        do {
            ...
            reporter.check("completion body preserves confirmed actual dose and override reason", (body?["completedItems"] as? [[String: Any]])?.first?["actualDose"] != nil && (body?["warningOverrides"] as? [[String: Any]])?.first?["reason"] as? String == "Cleared by PT")
    ...
    private static func runSetupCases() -> Int {
        ...
        Preferences.withDefaults(defaults) {
            Keychain.withBackend(backend) {
                let exitCode = GroundworkSetup.execute(
                ...
                reporter.check("setup persists only non-secret preferences", Preferences.groundworkLocationID == "office" && Preferences.groundworkDurationMinutes == 7 && !String(describing: defaults.persistentDomain(forName: suiteName)).contains("SENTINEL_SETUP_SECRET"))
